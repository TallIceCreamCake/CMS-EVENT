...
<img class="rounded-t-lg" src="../images/imagematos/<?php echo $matos_detail['image'] ?>" alt="" />
    ...
    <div class="basis-1/4 flex-0 ml-[1rem] mr-[10rem] p-4">
        ...
        <?php
            ...
            // Vérifiez si $row_stock est défini avant d'accéder à ses propriétés
            if ($row_stock !== null) {
...
while ($row_warehouse = $result_warehouse->fetch_assoc()) {
  ...
  // Check if $row_stock is not null before accessing its properties
  if ($row_stock !== null) {
      echo '<td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">' . $row_stock['nb_stockequipment'] . '</td>';
  } else {
      echo '<td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">N/A</td>';
  }

  echo '<td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
      <a href="../entrepot/details.php?entrepot=' . $row_warehouse['id'] . '" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Voir les détails</a>
  </td>';
  echo '</tr>';
  // Fetch the next row from $result_stock within the loop
  $row_stock = $result_stock->fetch_assoc();
}
                        ?>
                    </tbody>
